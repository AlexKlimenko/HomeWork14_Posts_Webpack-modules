...
function postTemplate(post, users, comments) {
  ...
  postComents.forEach(postComent => {
    ...
    <div class="card mb-2"> 
      <div class="card-body p-2">
        <p class="card-subtitle text-muted comment-name pb-2">${postComent.name}</p>
        <p class="card-subtitle text-muted comment-body m-0">${postComent.body}</p>
        <span class="card-subtitle d-block text-right comment-email pt-2">${postComent.email}</span>
      </div>
  ...
  return `
  <div class="col-md-6 card-container d-none">
    <div class="card mb-2">
      <div class="card-body">
        <h4 class="card-title">${post.title || ""}</h4>
        <p class="card-text">${post.body || ""}</p>
        <span class="d-block text-right">${user.username}</span>
        <button type="button" class="btn btn-outline-primary" data-toggle="modal" data-target="#userInfo_${post.id}">User info</button>
        <a class="btn btn-outline-primary btn-comments" data-toggle="collapse" href="#comment_${post.id}" aria-expanded="false" data-post-id=${post.id}>Comments</a>
        <div class="modal" id="userInfo_${post.id}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
              ...
              <div class="modal-body">
                <p class="card-subtitle text-muted user-name">Name: ${user.name}</p>
                <p class="card-subtitle text-muted user-email">Email: ${user.email}</p>
                <p class="card-subtitle text-muted user-website">Website: ${user.website}</p>
                <p class="card-subtitle text-muted user-phone">Phone: ${user.phone}</p>
              </div>
